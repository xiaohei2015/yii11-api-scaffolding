 ...
 * QQ登录
 */
class QqController extends HrController {
    ...
    public function actionLogin() {
        ...
        $qqCom = Yii::app()->qq;
        $qq = $qqCom::getInsatnce();
        return $qq->qq_login();
    }


    private function setQqBindState()
    {
    ...
    public function actionCallback()
    {
        ...
        $info = $qqCom::getUserInfo($access_token, $openid);
        $state = md5(uniqid(rand(), TRUE));
        //获取用户身份成功
        if ($info && isset($info['nickname'])) {
            $data = [];
            $data['appUserId'] = $openid;
            $data['nickname'] = $info['nickname'];
            $data['avatar'] = $info['figureurl_qq_2'];
            $data['thirdType'] = 2;
            Yii::app()->cache->set($state, $data, 60 * 60);

            //创建第三方信息
            $userAuth = UserAuths::model()->findByAttributes(['app_user_id' => $openid, 'app_type' => 2]);

            //创建新用户
            if (!$userAuth) {
                $userAuth = new UserAuths();
                $userAuth->app_user_id = $openid;
                ...
                $userAuth->nickname = $info['nickname'];
                $userAuth->avatar = $info['figureurl_qq_2'];
...
//             return $this->redirect(Yii::app()->params['hostUrl'].$this->redirectBindUrl);  
            ...
            //更新头像
            if ($userAuth->avatar <> $info['figureurl_qq_2'] || $userAuth->nickname <> $info['nickname']) {
                $userAuth->nickname = $info['nickname'];
                $userAuth->avatar = $info['figureurl_qq_2'];
                $userAuth->update_time = time();
                $userAuth->save();
...
//             return $this->redirect(Yii::app()->params['hostUrl'].$this->redirectBindUrl);  
    ...
    public function actionUnbind()
    {
        ...
        $userAuth = UserAuths::model()->findByAttributes(['id' => $thirdUserId, 'user_id' => $userId, 'app_type' => 2]);

        if (!$userAuth) {
            $this->outputJson($this->returnMsg('failed', '未绑定该QQ号，请重新解绑！'));
        }
